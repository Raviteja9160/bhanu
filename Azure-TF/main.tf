...
resource "azurerm_virtual_machine" "private_vm" {
  ...
  os_profile {
    computer_name  = "private_vm_${count.index}"
...
resource "azurerm_virtual_machine" "public_vm" {
  count                   = 2
  name                    = "public_vm_${count.index}"
  location                = "eastus"
  resource_group_name     = azurerm_resource_group.myresourcegroup.name
  network_interface_ids   = [azurerm_network_interface.public_nic.*.id[count.index]]
  vm_size                 = "Standard_B1s"
  storage_os_disk {
    name              = "public_os_disk_${count.index}"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
  os_profile {
    computer_name  = "public_vm_${count.index}"
    admin_username = "adminuser"
    admin_password = "Password1234!"
  }
  os_profile_linux_config {
